...
### People (Phase 5, `supabase_people_practices.sql`)

Every person is one row in `clients`, whatever they do: `practices text[]` holds `kinesiology` / `voice` / `piano` (existing clients default to kinesiology). Kinesiology-only lists — session pickers, client audit, clinical oversight, duplicate resolution, the Notion "sync all clients" — filter with `.contains("practices", ["kinesiology"])`. When someone books kinesiology under an email that already has a row, that row gains `kinesiology` (calcom-webhook, sync-calcom-bookings, voiceToFnh) instead of a second row being created. Voice/piano students are added by the `people-backfill` edge function (Settings → Data & Tools → "One list of people": preview, apply, undo; each change is logged in `people_backfill_log`). Backfilled rows carry `source = 'voice_backfill'` and `notion_voice_client_id` — never `notion_page_id`, which is the kinesiology Notion client page. The app is the main record for students; Notion Voice Clients only receives copies. New students get a row the first time a lesson is scheduled (`_shared/people.ts` `ensurePerson`, called from voice-schedule-lesson). People and Follow-up show one entry per person: kinesiology sessions and voice/piano lessons (matched by email) form one history and one status. Voice-only people still open the voice hub by `voice:<email>` until the Assistant moves to client ids. Two statuses per person: **stage** (`src/lib/clientStatus.ts`: lead / active / at risk / lapsed, computed, plus `closed` set by hand — "Close — don't follow up" in Follow-up, the Client audit Follow-up column, reopen from People; `src/lib/people.ts` `setPersonClosed`; closed people are left out of Follow-up, the Assistant's needing-attention tool and pipeline counts) and **conversation** (needs reply / waiting on them from the thread, Done / Booked marks in `inbox_contact_state` by email — `saveContactMark` in `src/lib/inbox-marks.ts`, used by the Inbox and every email thread). The old warm/cold/lost tag and `client_email_status` are retired (`supabase_people_closed_stage.sql` moved "lost" to closed).
