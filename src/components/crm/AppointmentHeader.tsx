...
const AppointmentHeader = ({ appointment, onSaveField, onUpdate }: AppointmentHeaderProps) => {
  ...
  const VitalCard = ({ icon: Icon, label, value, subValue, color, onClick, children }: any) => (
    <div 
      onClick={onClick}
      className={cn(
        "flex-1 min-w-[140px] p-4 rounded-2xl border transition-all duration-300 group cursor-pointer",
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className={cn(
          "w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-500 group-hover:scale-110",
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="flex items-start gap-6">
          <div className="relative shrink-0">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-[2.5rem] bg-indigo-600 text-white flex items-center justify-center text-3xl md:text-4xl font-black shadow-2xl shadow-indigo-200 dark:shadow-none">
          ...
          <div className="space-y-3 min-w-0">
            <div className="flex flex-wrap items-center gap-4">
              <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tighter truncate">
                {appointment.clients.name}
              </h1>
              <div className="flex items-center gap-2">
                {clientBorn && (
                  <Badge variant="outline" className="h-7 px-3 text-xs font-black border-indigo-100 text-indigo-600 bg-indigo-50/50 rounded-xl">
            ...
            <div className="flex flex-wrap items-center gap-x-8 gap-y-3">
              <div className="flex items-center gap-6">
                ...
                >
                  <div className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center text-indigo-400 group-hover:bg-indigo-50 group-hover:scale-110 transition-all shadow-sm">
                    <Calendar size={16} />
                  </div>
                  <span className="text-sm font-bold text-slate-600">{format(appointment.date, "EEEE, MMM d")}</span>
                </button>
                <button 
                  onClick={() => setRescheduleOpen(true)}
                  className="flex items-center gap-2.5 hover:text-indigo-600 transition-all group"
                >
                  ...
                  <span className="text-sm font-bold text-slate-600">{format(appointment.date, "h:mm a")}</span>
                </button>
              </div>
              ...
              <div className="flex items-center gap-3">
                <Select value={appointment.status} onValueChange={(newStatus) => onSaveField('status', newStatus)}>
                  <SelectTrigger className={cn(
                    "h-10 w-[150px] text-xs font-black uppercase tracking-widest border-slate-200 bg-white rounded-xl shadow-sm",
                    appointment.status === 'Completed' ? "text-emerald-600 border-emerald-100" : "text-indigo-600"
                  )}>
                    <SelectValue placeholder="Status" />
                  </SelectTrigger>
                  <SelectContent className="rounded-2xl border-none shadow-3xl p-1">
                    {APPOINTMENT_STATUSES.map(status => (
                      <SelectItem 
                        key={status} 
                        value={status}
                        className="rounded-xl text-[10px] font-bold uppercase tracking-wider py-3 px-5 cursor-pointer"
                      >
                        {status}
                      </SelectItem>
                    ))}
        ...
        <div className="flex flex-wrap items-center gap-4 p-3 bg-slate-100/50 rounded-[2.5rem] border border-slate-100 shadow-inner">
          <VitalCard 
            icon={ShieldAlert} 
            label="Neural Load" 
            value={`${neuralLoad}%`} 
          ...
          >
            <div className={cn("w-2 h-2 rounded-full animate-pulse", neuralLoad > 50 ? "bg-rose-500" : "bg-indigo-400")} />
          </VitalCard>
          ...
          >
            {appointment.is_paid && (
              <div className={cn("w-2 h-2 rounded-full", appointment.payment_received ? "bg-emerald-500" : "bg-rose-500")} />
            )}
          </VitalCard>
