...
const BrainZonePrintable = () => {
    ...
    return (
      <div className="border border-black p-1.5 flex flex-col h-full break-inside-avoid bg-white">
        <div className="flex items-center justify-between mb-1 border-b border-black/10 pb-0.5">
          <h4 className="font-black text-[9px] uppercase truncate leading-none">{point.name}</h4>
          <span className={cn("text-[7px] font-black px-1 rounded-sm text-white leading-none", color)}>
            {point.acupoint || point.category[0]}
          </span>
        </div>
        
        <div className="relative aspect-video bg-slate-50 border border-slate-100 mb-1 overflow-hidden flex items-center justify-center">
          {images?.primary ? (
        ...
        <div className="space-y-0.5 text-[7px] leading-tight">
          <p className="line-clamp-1"><strong>Loc:</strong> {point.location}</p>
          <p className="line-clamp-1"><strong>Stim:</strong> {point.stimulus || point.technique || "Standard"}</p>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="bg-white text-black p-2 max-w-[297mm] mx-auto font-sans print:p-0 print:m-0">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 border-b border-purple-600 pb-0.5">
            ...
            <h2 className="text-[10px] font-black uppercase text-purple-600">Cortical Zones (Contralateral)</h2>
          </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2 border-b border-indigo-600 pb-0.5">
            ...
            <h2 className="text-[10px] font-black uppercase text-indigo-600">Subcortical Zones (Ipsilateral)</h2>
          </div>
          <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 gap-2">
      ...
      <div className="mt-4 pt-1 border-t border-slate-200 flex justify-between items-center">
