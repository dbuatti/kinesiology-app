...
const BrainstemBreathingReference = () => {
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="p-8 bg-foreground text-primary-foreground rounded-2xl shadow-2xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-10"><Wind size={150} /></div>
        <div className="relative z-10 space-y-4">
          <Badge className="bg-teal-500 text-primary-foreground border-none font-semibold text-[10px] uppercase tracking-wider px-4 py-1">Neurological Input</Badge>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {BRAINSTEM_BREATHING.map((item) => (
          <Card key={item.region} className="border-none shadow-xl rounded-2xl bg-card overflow-hidden group hover:shadow-2xl transition-all duration-500">
            <div className={cn("p-8 text-primary-foreground", item.color)}>
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-2xl font-semibold">{item.region}</h4>
                <Badge className="bg-card/20 text-primary-foreground border-none font-semibold text-[10px] uppercase tracking-wider px-3 py-1">
                  {item.tone} Tone
                </Badge>
              </div>
              <p className="text-xs font-bold opacity-90 uppercase tracking-wider mb-4">{item.nerves}</p>
                ...
                {item.patterns.map((p) => (
                  <div key={p.name} className={cn("p-5 rounded-2xl border-2 transition-all group/item", item.light, item.border)}>
                    <div className="flex items-center justify-between mb-2">
                      <p className={cn("text-sm font-semibold uppercase tracking-tight", item.text)}>{p.name}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card className="border-none shadow-lg rounded-2xl bg-indigo-900 text-primary-foreground overflow-hidden">
          ...
          <CardContent className="p-8 pt-0 space-y-6">
            <div className="grid grid-cols-1 gap-4">
              <div className="p-4 bg-card/5 rounded-2xl border border-primary-foreground/10">
                <h5 className="font-semibold text-primary text-xs uppercase tracking-wider mb-2">Flexor Dominance?</h5>
              ...
              <div className="p-4 bg-card/5 rounded-2xl border border-primary-foreground/10">
                <h5 className="font-semibold text-rose-400 text-xs uppercase tracking-wider mb-2">Extensor Dominance?</h5>
        ...
        <div className="p-8 bg-primary/5 rounded-2xl border-2 border-primary/20 flex items-start gap-6">
          ...
          <div className="space-y-2">
            <h4 className="text-xl font-semibold text-primary">Clinical Safety Note</h4>
