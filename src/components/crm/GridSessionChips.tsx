...
export const GridSessionChips = ({ grid }: { grid?: ClientGridData }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 pt-0.5">
      <span className="inline-flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider text-chart-primary">
      ...
      {grid.muscleCount > 0 && (
        <>
          ...
          <span className="inline-flex items-center gap-1" title="Intrinsic muscles with tone findings">
            ...
            <span className="text-[10px] font-bold tabular-nums text-muted-foreground">{grid.muscleCount}</span>
          </span>
        </>
      )}
    </div>
