...
const HeartWallBible = () => {
  return (
    <div className="space-y-12 animate-in fade-in duration-700">
      ...
      {/* The Emperor Quote */}
      <div className="p-8 bg-indigo-900 rounded-[2.5rem] shadow-lg text-white relative overflow-hidden">
      ...
      <div className="p-8 bg-rose-50 rounded-[2.5rem] border-2 border-rose-100 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
          {[
            ...
            <div key={item.t} className="p-4 bg-white rounded-2xl border border-rose-200">
              <p className="font-black text-sm text-rose-700">{item.t}</p>
              <p className="text-xs text-slate-600 font-medium mt-1 leading-relaxed">{item.d}</p>
            </div>
          ))}
        </div>
      </div>
