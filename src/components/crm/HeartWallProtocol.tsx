...
const HeartWallProtocol = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 animate-in fade-in duration-700 pb-32 font-serif">
      {/* Header */}
      <div className="text-center space-y-4 mb-20">
        <Badge className="bg-rose-100 text-rose-700 border-none font-black text-[10px] uppercase tracking-[0.4em] px-6 py-2 rounded-full font-sans">
          Clinical Reference
        </Badge>
        ...
        <div className="space-y-8">
          {ASSESSMENT_STEPS.map((step) => (
            <div key={step.id} className="flex gap-6 group">
              <span className="text-2xl font-black text-indigo-200 group-hover:text-indigo-600 transition-colors tabular-nums font-sans">
                0{step.id}
              </span>
      ...
      <section className="mt-24">
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-slate-900 text-white font-sans">
                <th className="p-6 text-left uppercase tracking-widest text-[10px] border-r border-white/10">Organ</th>
                <th className="p-6 text-left uppercase tracking-widest text-[10px] border-r border-white/10">Emotions</th>
                <th className="p-6 text-left uppercase tracking-widest text-[10px]">Muscles</th>
              </tr>
            </thead>
              ...
              {[1, 2, 3, 4, 5, 6].map((rowNum) => (
                ...
                >
                  <td className="p-6 border-r border-slate-100 align-top">
                    <p className="font-black text-indigo-600 text-base leading-tight">
        ...
        {selectedRow && (
          <div className="mt-8 p-8 bg-slate-900 text-white rounded-[2.5rem] shadow-xl animate-in zoom-in-95 duration-300 relative overflow-hidden">
            <div className="absolute top-0 right-0 p-6 opacity-10"><Sparkles size={100} /></div>
            <div className="flex items-center justify-between mb-6 relative z-10">
              <div className="space-y-1">
                <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] font-sans">Active Focus</p>
                <h4 className="text-2xl font-bold">{ROW_DATA[selectedRow].organ}</h4>
              </div>
              <Button variant="ghost" size="sm" onClick={() => setSelectedRow(null)} className="text-indigo-300 hover:text-white font-sans">
                <RefreshCw size={16} className="mr-2" /> Clear
              </Button>
            </div>
            <div className="space-y-4 relative z-10">
              <div className="flex items-start gap-3">
                <Dumbbell size={18} className="text-indigo-400 shrink-0 mt-1" />
      ...
      <section className="mt-24">
        ...
        <div className="space-y-10">
          {CORRECTION_STEPS.map((step) => (
            <div key={step.id} className="flex gap-6 group">
              <span className="text-2xl font-black text-emerald-200 group-hover:text-emerald-600 transition-colors tabular-nums font-sans">
                0{step.id}
              </span>
              <div className="space-y-1">
                <h4 className="text-xl font-bold text-slate-900">{step.title}</h4>
