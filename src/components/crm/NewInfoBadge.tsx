...
const NewInfoBadge = ({ submittedAt, className = "" }: { submittedAt?: string | null; className?: string }) => {
  ...
  return (
    <span
      ...
      className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-500 text-primary-foreground text-[10px] font-semibold uppercase tracking-wider shadow-sm ${className}`}
    >
      <Sparkles size={9} /> New
    </span>
  );
