...
const PathwayAssessmentWizard = ({ onSave, initialValue }: PathwayAssessmentWizardProps) => {
  ...
  const renderImagePreview = (pointId: string) => {
    ...
    if (!images?.secondaryUrl) return null;

    return (
      <div className="mt-2 aspect-square rounded-xl overflow-hidden border-2 border-slate-200 bg-slate-50 shadow-sm">
        <img src={images.secondaryUrl} alt="Reflex Point" className="w-full h-full object-cover" />
      </div>
    );
  };
          ...
          >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {cranialNerves.map((nerve) => {
                ...
                return (
                  <div key={nerve.id} className="space-y-2">
                    <div className={cn(
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <h4 className="font-black text-sm text-slate-900">{nerve.name}</h4>
                          {result && (
                            <div className={cn(
                    ...
                    {cortical.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Badge variant="outline" className="text-[8px] font-black uppercase border-purple-200 text-purple-600 w-full justify-center">
                            {p.lateralization}
                          </Badge>
                        </button>
                      );
                    ...
                    {subcortical.map((p) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <Badge variant="outline" className="text-[8px] font-black uppercase border-indigo-200 text-indigo-600 w-full justify-center">
                            {p.lateralization}
                          </Badge>
                        </button>
                      );
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-4">
              <Button className="h-16 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-xl font-black shadow-lg shadow-emerald-100" onClick={() => { setState({ ...state, reassessed: true }); handleComplete(); }}>
                {state.pathway === 'Cranial Nerve' ? 'Complete Assessment' : 'Pathway is Clear'} <CheckCircle2 size={24} className="ml-2" />
              </Button>
              {state.pathway !== 'Cranial Nerve' && (
            ...
            <Button variant="ghost" onClick={() => prevStep(state.pathway === 'Cranial Nerve' ? 'SELECT_CRANIAL_NERVE' : (state.response === 'Clear/Normal' ? 'SELECT_RESPONSE' : 'CORRECTION'))} className="w-full h-12 rounded-xl"><ChevronLeft size={18} className="mr-2" /> Back</Button>
          </motion.div>
        );
          ...
          <div className="flex flex-wrap items-center gap-2 mb-8 p-3 bg-slate-50 rounded-2xl border border-slate-100 animate-in fade-in slide-in-from-top-2">
            ...
            {state.pathway && (
              <div className="flex items-center gap-1.5">
                ...
                {(state.selectedMuscle || state.selectedBrainZone || state.cranialNerveResults.length > 0) && <ArrowRight size={10} className="text-slate-300" />}
              </div>
            )}
