      ...
      <div className="grid grid-cols-2 gap-4">
        {['Left', 'Right'].map(side => (
          <div key={side} className="space-y-2">
            <p className="text-[10px] font-semibold text-center uppercase text-muted-foreground">{side} Wrist</p>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col gap-0.5">
                      <span className={cn(isMatch && !isDeep ? "text-primary-foreground" : "")}>Light: {p.light}</span>
                      <span className={cn(isMatch && isDeep ? "text-primary-foreground" : "")}>Deep: {p.deep}</span>
                    </div>
                  </div>
                );
              })}
