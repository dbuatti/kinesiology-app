...
const SessionToolNavigation = ({ 
  ...
  const NavItem = ({ view, label, Icon }: { view: ActiveView, label: string, Icon: ElementType }) => (
    <Button
      ...
      className={cn(
        "h-9 px-3.5 rounded-lg font-medium text-[13px] shrink-0 gap-2.5 text-foreground",
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full md:w-auto px-1">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              className={cn(
                "h-9 px-3.5 rounded-lg transition-all font-medium text-[13px] shrink-0 flex items-center gap-2.5 text-foreground hover:bg-accent hover:text-accent-foreground",
