...
import { ChevronDown, Zap, RotateCcw, Loader2 } from "lucide-react";
...
const T1SympatheticReset = ({ 
  ...
  const handleReset = async () => {
    ...
    setLoading(true);
    try {
      await supabase.from("appointments").update({ t1_reset_notes: null }).eq("id", appointmentId);
      ...
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
        <CollapsibleTrigger asChild>
          <div className={cn(
            ...
            <div className="flex items-center gap-3">
              <Zap size={14} className={cn(isOpen ? "text-indigo-400" : "text-primary")} />
              <span className="text-[11px] font-black uppercase tracking-widest">T1 Reset</span>
            </div>
            <div className="flex items-center gap-3">
              {initialNotes && (
        ...
        <CollapsibleContent>
          <div className="p-4 border-t border-slate-100 space-y-4 animate-in fade-in slide-in-from-top-1 duration-200">
            <Textarea 
              value={initialNotes || ""} 
              onChange={(e) => onSaveField('t1_reset_notes', e.target.value)}
