...
const ClinicalDashboard = ({ stats, todaySessions, activeSession, morningProgress }: ClinicalDashboardProps) => {
  const missions = [
    { label: "Grounding Ritual", status: morningProgress >= 25 ? 'done' : 'pending', icon: Sparkles },
    { label: "Clinical Alerts", status: stats.imperativeAlerts === 0 ? 'done' : 'pending', icon: ShieldAlert },
    { label: "Session Preparation", status: 'pending', icon: ClipboardCheck },
  ];
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-end justify-between gap-6 border-b-2 border-slate-900 pb-6">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="h-12 px-8 border-2 border-slate-900 font-black text-[10px] uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all shadow-lg">
            Switch Workspace
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4">
          <Card className="h-full border-none shadow-2xl rounded-none bg-navy text-white overflow-hidden relative group">
            ...
            <CardContent className="p-10 flex flex-col justify-between h-full relative z-10">
              <div className="space-y-8">
                <div className="space-y-2">
                  <p className="text-[10px] font-black uppercase tracking-[0.4em] text-indigo-400">Daily Mission</p>
                  <h3 className="text-3xl font-serif font-bold">Clinical Readiness</h3>
                </div>
                
                <div className="space-y-4">
                  {missions.map((m, i) => (
                    <div key={i} className={cn(
                      "flex items-center justify-between p-4 transition-colors border border-white/5",
                      ...
                      {m.status === 'done' ? <CheckCircle2 size={18} /> : <div className="w-4 h-4 border-2 border-white/20" />}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="mt-12 pt-8 border-t border-white/10">
                ...
                <Progress value={morningProgress} className="h-2 bg-white/10 [&>div]:bg-indigo-500" />
              </div>
            </CardContent>
          </Card>
        </div>
