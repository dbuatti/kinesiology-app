...
const DocInput = ({ label, value, field, placeholder, multiline = false, type = "text", onChange }: DocInputProps) => {
  return (
    <div className="space-y-1.5 group">
      <label className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground group-focus-within:text-foreground transition-colors">{label}</label>
      {multiline ? (
        <textarea
          value={value || ""}
