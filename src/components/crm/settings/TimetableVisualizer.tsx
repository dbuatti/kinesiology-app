...
const TimetableVisualizer = ({ clients }: TimetableVisualizerProps) => {
  ...
  if (activeClients.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-4 text-center">
        ...
        <p className="text-sm font-bold text-muted-foreground">No active clients to schedule.</p>
    ...
    return (
      <td
        ...
        className={cn(
          ...
          cell.hasConflict && !isOver ? "bg-rose-50/40 dark:bg-rose-950/10" : "",
          ...
          <div className="space-y-1 pt-0.5">
            {cell.clients.map(client => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "group flex items-center gap-1 px-1.5 py-1 rounded-lg text-[11px] font-bold border select-none transition-all",
                    ...
                    cell.hasConflict
                      ...
                      : "bg-primary/8 text-primary border-primary/15 dark:bg-primary/10 dark:border-primary/20"
                  )}
                ...
                >
                  ...
                  <span className="shrink-0 text-[9px] font-black opacity-50">${client.standard_rate ?? 50}</span>
                  {isPending && <span className="w-1.5 h-1.5 rounded-full bg-violet-500 shrink-0" title="Unsaved" />}
            ...
            {isEmpty && isDragging && (
              <div className="mx-0.5 h-6 rounded-lg border-2 border-dashed border-primary/20 flex items-center justify-center">
                <span className="text-[9px] font-bold text-primary/30 uppercase tracking-widest">Drop</span>
              </div>
            )}
            {isEmpty && !isDragging && (
              <span className="block text-center text-[10px] font-bold text-muted-foreground/30 uppercase tracking-widest pt-2">Open</span>
            )}
          </div>
        )}
      </td>
  ...
  const renderWeekGrid = (week: 1 | 2) => {
    ...
    return (
      <div className="bg-card rounded-[2rem] border border-border shadow-lg overflow-hidden">
        {/* Week header */}
        <div className="px-5 py-3.5 bg-muted/40 border-b border-border flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="font-black text-sm text-foreground">Week {week}</h4>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {DAYS.map(day => dailyBreakdown[day] > 0 && (
              <div key={day} className="px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/30 text-center">
                <span className="text-[9px] font-bold text-muted-foreground block leading-tight">{day.substring(0, 3)}</span>
                <span className="text-[10px] font-black text-emerald-700 dark:text-emerald-400">${dailyBreakdown[day]}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                <th className="p-3 pl-5 font-black text-[10px] uppercase tracking-widest text-muted-foreground w-24 text-left">Time</th>
                {DAYS.map(day => (
                  <th key={day} className="p-3 font-black text-[10px] uppercase tracking-widest text-muted-foreground text-center">
                    {day.substring(0, 3)}
            ...
            <tbody>
              ...
              {MORNING_SLOTS.map(slot => (
                <tr key={slot} className="border-b border-border/40 hover:bg-muted/5 transition-colors">
                  <td className="p-2 pl-5 font-black text-xs text-muted-foreground bg-muted/10 whitespace-nowrap align-middle">
                    {TIME_LABELS[slot]}
                  </td>
              ...
              <tr className="border-b border-border/40">
                <td colSpan={6} className="py-2 px-5 bg-muted/30">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground flex items-center gap-1.5 shrink-0">
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap justify-end">
          ...
          {!isLocked && pendingSaves.size > 0 && (
            <>
              ...
              <Button size="sm" onClick={handleSaveAll} disabled={isSaving}
                className="rounded-xl text-xs font-bold bg-primary hover:bg-primary/90 text-primary-foreground h-9 px-5">
          ...
          {!isLocked && scheduledData.conflictsList.length > 0 && (
            <Button onClick={() => setIsOptimized(!isOptimized)} size="sm"
              className={cn(
                "rounded-xl h-9 px-5 font-black text-[10px] uppercase tracking-widest",
                isOptimized
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <Card key={label} className="border-none shadow-md rounded-[2rem] bg-card overflow-hidden col-span-1">
            <CardContent className="p-5 space-y-2">
              ...
              <h3 className="text-2xl font-black text-foreground">${fmt(value)}</h3>
              <p className="text-[10px] text-muted-foreground font-medium">{cap} {sub} · {MAX_WEEKLY_SLOTS} max slots</p>
            </CardContent>
          </Card>
        ))}
        <Card className="border-none shadow-lg rounded-[2rem] bg-primary text-primary-foreground overflow-hidden col-span-1">
          <CardContent className="p-5 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase tracking-widest opacity-70">Fortnightly</span>
              <div className="w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center">
                <Sparkles size={14} />
              </div>
            </div>
            <h3 className="text-2xl font-black">${fmt(earningsSummary.fortnight)}</h3>
      ...
      <div className="flex flex-wrap items-center gap-4 px-1">
        {[
          ...
          { cls: "bg-violet-100 border-violet-200 dark:bg-violet-950/30 dark:border-violet-800/40", label: "Manually moved" },
          { cls: "bg-destructive/10 border-destructive/20", label: "Conflict" },
        ].map(({ cls, label }) => (
          <div key={label} className="flex items-center gap-2 text-[10px] font-bold text-muted-foreground">
      ...
      {isOptimized && optimizedData.proposedMoves.length > 0 && (
        <Card className="border-none shadow-lg rounded-[2rem] bg-card border border-emerald-200 dark:border-emerald-900/40 overflow-hidden animate-in slide-in-from-top-4 duration-500">
          <CardHeader className="p-6 pb-4 bg-emerald-50/50 dark:bg-emerald-950/20 border-b border-emerald-100 dark:border-emerald-900/30">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="space-y-1">
                <CardTitle className="text-base font-black text-emerald-900 dark:text-emerald-400 flex items-center gap-2">
                  <Sparkles size={18} className="text-emerald-600" /> Auto-Resolved Conflicts
                </CardTitle>
                <CardDescription className="text-emerald-700 dark:text-emerald-500 font-medium">
              ...
              <Button onClick={handleApplyOptimizedSchedule} disabled={applyingMoves}
                className="bg-emerald-600 hover:bg-emerald-700 dark:bg-emerald-700 dark:hover:bg-emerald-600 text-white rounded-xl h-9 px-6 font-black text-[10px] uppercase tracking-widest shrink-0">
          ...
          <CardContent className="p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {optimizedData.proposedMoves.map(move => (
                <div key={move.clientId} className="p-3.5 bg-muted/30 rounded-2xl border border-border space-y-1.5">
                  <p className="font-bold text-foreground text-sm">{move.clientName}</p>
                  <p className="text-[10px] text-muted-foreground font-medium">
                    <span className="text-destructive font-bold">{move.originalDay} {TIME_LABELS[move.originalSlot]} W{move.originalWeek}</span>
                    {" → "}
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">{move.newDay} {TIME_LABELS[move.newSlot]} W{move.newWeek}</span>
                  </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Insights */}
        <Card className="border-none shadow-lg rounded-[2rem] bg-slate-900 dark:bg-slate-950 text-white overflow-hidden relative">
          ...
          <CardContent className="p-5 pt-0 space-y-2">
            {suggestions.length > 0 ? suggestions.map((s, i) => (
              ...
              <div className="text-center py-8 space-y-2">
                ...
                <p className="text-xs font-bold text-slate-300">Schedule looks great!</p>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card
          className={cn(
            "border-none shadow-lg rounded-[2rem] bg-card overflow-hidden transition-all duration-150",
          ...
          <CardHeader className="pb-2 border-b border-border bg-muted/30">
            <CardTitle className="text-xs font-black uppercase tracking-widest text-muted-foreground flex items-center gap-2">
              ...
              {dragOverKey === "unscheduled" && (
                <span className="text-[10px] text-primary font-bold ml-1">· Drop to remove</span>
              )}
            </CardTitle>
          </CardHeader>
                  ...
                  {scheduledData.unscheduled.map(client => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <span className="text-xs font-bold text-foreground truncate">{client.name}</span>
                        <span className="text-[9px] font-black text-muted-foreground">${client.standard_rate ?? 50}</span>
                        {pendingSaves.has(client.id) && <span className="w-1.5 h-1.5 rounded-full bg-violet-500 shrink-0" />}
                      </div>
        ...
        {Object.keys(manualPlacements).length > 0 ? (
          <Card className="border-none shadow-sm rounded-[2rem] bg-card overflow-hidden border border-violet-200 dark:border-violet-900/40">
            <CardHeader className="pb-2 border-b border-border bg-muted/30">
              <CardTitle className="text-xs font-black uppercase tracking-widest text-muted-foreground flex items-center justify-between gap-2">
          ...
          <Card className="border-none shadow-sm rounded-[2rem] bg-card overflow-hidden border border-border">
            <CardContent className="p-5 flex flex-col items-center justify-center h-full text-center space-y-2 py-10">
              ...
              <p className="text-xs font-bold text-muted-foreground">No manual changes yet.</p>
