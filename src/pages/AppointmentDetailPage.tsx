...
import { 
  ArrowLeft, Calendar, Clock, Target, Zap, Activity, 
  ExternalLink, Loader2, Trash2, Edit3, User, FlaskConical
} from "lucide-react";
import { format } from "date-fns";
import { Appointment } from "@/types/crm";
import { showSuccess, showError } from "@/utils/toast";
import { cn } from "@/lib/utils";
import BoltTestCard from "@/components/crm/BoltTestCard";
import {
  Dialog,
  DialogContent,
  DialogHeader,
        ...
        <div className="flex gap-2">
          <Dialog open={editOpen} onOpenChange={setEditOpen}>
            ...
            <DialogContent className="sm:max-w-[500px]">
              ...
              {/* Note: AppointmentForm currently only supports creation, but we can reuse it for editing later */}
              <AppointmentForm 
                onSuccess={() => {
                  setEditOpen(false);
                  fetchAppointmentData();
                }} 
                // We would pass initial data here if the form supported editing
              />
            </DialogContent>
          </Dialog>
          <Button 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-1 space-y-6">
              ...
              <Card className="border-none shadow-sm rounded-2xl bg-white">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2 text-slate-700">
                    <User size={16} className="text-indigo-500" /> Client Quick Link
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <Link to={clientLink}>
                    <Button variant="outline" className="w-full justify-between h-12 rounded-xl border-slate-200 hover:bg-slate-50">
                      <span className="font-bold text-slate-900">{appointment.clients.name}</span>
