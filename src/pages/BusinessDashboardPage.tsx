...
const ACTIONS = [
  { label: "Financial Overview", desc: "Deep-dive revenue metrics, charts, and projections.", icon: BarChart3, path: "/business/overview", color: "text-indigo-600", bg: "bg-indigo-50 dark:bg-indigo-900/20" },
  { label: "Client Audit", desc: "Review rates, recency, and AI pricing suggestions.", icon: FileText, path: "/business/client-audit", color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-900/20" },
  { label: "Marketing Engine", desc: "Transform clinical wins into newsletter assets.", icon: Target, path: "/business/marketing-engine", color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-900/20" },
];
...
const BusinessDashboardPage = () => {
  ...
  const miniCard = (label: string, value: string, sub: string, icon: any, accent: string) => (
    <Card className="border-none shadow-lg rounded-[1.5rem] bg-card overflow-hidden">
      <CardContent className="p-5 flex items-center gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {loading ? <Skeleton className="h-7 w-20 rounded-lg mt-1" /> : <p className="text-xl font-black text-foreground tracking-tight">{value}</p>}
          {!loading && sub && <p className="text-[10px] font-bold text-muted-foreground truncate">{sub}</p>}
        </div>
      </CardContent>
    </Card>
  );
    ...
    <AppLayout>
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-16">
        <PageHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              }} disabled={backfilling} variant="outline" size="sm" className="rounded-xl h-11 px-5 border-border font-bold text-[10px] uppercase tracking-widest">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {miniCard("Total Revenue", fmt(v.total), v.sub, <DollarSign size={18} />, "bg-indigo-50 text-indigo-600 dark:bg-indigo-900/30")}
            {miniCard("Sessions", String(v.sessions), `${v.count} paying client${v.count !== 1 ? "s" : ""}`, <Activity size={18} />, "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30")}
            {miniCard("Monthly Run Rate", fmtShort(v.runRate.monthly) + "/mo", `${fmtShort(v.runRate.annual)}/yr projected`, <PiggyBank size={18} />, "bg-purple-50 text-purple-600 dark:bg-purple-900/30")}
            {miniCard("Paying Clients", String(v.count), viewMode === "combined" ? `Kin ${totals.kineCount} · Voice ${totals.voiceCount}` : "", <Users size={18} />, "bg-amber-50 text-amber-600 dark:bg-amber-900/30")}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
            <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
              <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
                <CalendarDays size={14} className="text-indigo-500" /> Upcoming Kinesiology
              </CardTitle>
            </CardHeader>
            <CardContent className="p-5">
              {loading ? (
                ...
                <p className="text-center py-6 text-sm font-bold text-muted-foreground">Switch to FNH or Combined view.</p>
              ) : upcomingAppts.length === 0 ? (
                <p className="text-center py-6 text-sm font-bold text-muted-foreground">No upcoming appointments.</p>
              ) : (
                <div className="space-y-2">
                  {upcomingAppts.map(a => (
                    <Link key={a.id} to={`/appointments/${a.id}`} className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border hover:bg-card hover:shadow-md transition-all group">
                      <div className="min-w-0 flex-1">
                        <p className="font-bold text-sm text-foreground truncate group-hover:text-indigo-600 transition-colors">{(a.clients as any)?.name || "Unknown"}</p>
                        <p className="text-[10px] font-medium text-muted-foreground">{format(parseISO(a.date), "EEE, MMM d · h:mm a")}</p>
                      </div>
                      {a.price_amount && <span className="text-xs font-black text-foreground shrink-0 ml-3">{fmt(Number(a.price_amount))}</span>}
                    </Link>
                  ))}
                </div>
              )}
          ...
          <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
            <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
              <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
                <Music size={14} className="text-rose-500" /> Upcoming Voice Lessons
              </CardTitle>
            </CardHeader>
            <CardContent className="p-5">
              {loading ? (
                ...
                <p className="text-center py-6 text-sm font-bold text-muted-foreground">Switch to Voice or Combined view.</p>
              ) : upcomingLessons.length === 0 ? (
                <p className="text-center py-6 text-sm font-bold text-muted-foreground">No upcoming lessons.</p>
              ) : (
                <div className="space-y-2">
                  {upcomingLessons.map(l => (
                    <div key={l.id} className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border">
                      <div className="min-w-0 flex-1">
                        <p className="font-bold text-sm text-foreground truncate">{l.student_name || "Unknown"}</p>
                        <p className="text-[10px] font-medium text-muted-foreground">{format(parseISO(l.lesson_date), "EEE, MMM d · h:mm a")}</p>
                      </div>
                      {l.cost && <span className="text-xs font-black text-foreground shrink-0 ml-3">{fmt(Number(l.cost))}</span>}
                    </div>
                  ))}
                </div>
              )}
        ...
        <div>
          <div className="flex items-center gap-3 px-1 mb-6">
            <div className="w-9 h-9 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shadow-sm"><Target size={16} /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {ACTIONS.map(a => (
              <Link key={a.path} to={a.path} className="block group">
                <Card className="border-none shadow-md rounded-[2rem] bg-card hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden h-full">
                  <CardContent className="p-6 space-y-4">
                    <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-sm", a.bg, a.color)}>
                      <a.icon size={22} />
                    </div>
                    <div className="space-y-1">
                      <h3 className="font-black text-foreground group-hover:text-blue-600 transition-colors">{a.label}</h3>
                      <p className="text-xs font-medium text-muted-foreground leading-relaxed">{a.desc}</p>
                    </div>
                    <div className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-muted-foreground group-hover:text-blue-600 transition-colors pt-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {viewMode !== "voice" && (
            <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
              <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
                <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
                  <User size={14} className="text-indigo-500" /> Top Kinesiology Clients
                </CardTitle>
              </CardHeader>
              <CardContent className="p-5">
                {loading ? (
                  ...
                  <p className="text-center py-6 text-sm font-bold text-muted-foreground">No paid sessions yet.</p>
                ) : (
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
                    {filteredKine.slice(0, 10).map(c => (
                      <Link key={c.id} to={`/clients/${c.id}`} className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border hover:bg-card hover:shadow-md transition-all group">
                        <div className="min-w-0 flex-1">
                          <p className="font-bold text-sm text-foreground truncate group-hover:text-indigo-600 transition-colors">{c.name}</p>
                        ...
                        <div className="text-right shrink-0 ml-3">
                          <p className="font-black text-sm text-foreground">{fmt(Math.round(c.totalSpent))}</p>
                          <p className="text-[9px] font-bold text-muted-foreground">{c.email || "no email"}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
          ...
          {viewMode !== "fnh" && (
            <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
              <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
                <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
                  <User size={14} className="text-rose-500" /> Top Voice Students
                </CardTitle>
              </CardHeader>
              <CardContent className="p-5">
                {loading ? (
                  ...
                  <p className="text-center py-6 text-sm font-bold text-muted-foreground">No paid lessons yet.</p>
                ) : (
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
                    {filteredVoice.map((s, i) => (
                      <div key={s.email || i} className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border hover:bg-card hover:shadow-md transition-all group">
                        ...
                        <div className="text-right shrink-0 ml-3">
                          <p className="font-black text-sm text-foreground">{fmt(Math.round(s.totalSpent))}</p>
                          <p className="text-[9px] font-bold text-muted-foreground">{s.email || "no email"}</p>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
          <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
            <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
              <Clock size={14} className="text-blue-500" /> Recent Payments
            </CardTitle>
          </CardHeader>
          <CardContent className="p-5">
            {loading ? (
              ...
              <p className="text-center py-6 text-sm font-bold text-muted-foreground">No payments yet.</p>
            ) : (
              <div className="space-y-1">
                  ...
                  return (
                    <div key={r.id || i} className="flex items-center justify-between p-3 rounded-xl hover:bg-muted/30 transition-colors">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0", isKine ? "bg-indigo-50 text-indigo-600" : "bg-rose-50 text-rose-600")}>
                          <DollarSign size={14} />
                        </div>
                        <div className="min-w-0">
                          <div className="flex items-center gap-2">
                            {isKine && clientId ? (
                              <Link to={`/clients/${clientId}`} className="font-bold text-sm text-foreground truncate hover:text-indigo-600 transition-colors">{name}</Link>
                            ) : (
                          ...
                          <p className="text-[10px] font-medium text-muted-foreground">
                            ...
                            <span className={cn("ml-2 font-black", isKine ? "text-indigo-500" : "text-rose-500")}>{isKine ? "Kine" : "Voice"}</span>
                          </p>
                        </div>
                      </div>
                      <span className="font-black text-sm text-foreground shrink-0 ml-3">{fmt(amount)}</span>
                    </div>
                  );
                })}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
            <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
              <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
                <Mail size={14} className="text-blue-500" /> Communication Summary
              </CardTitle>
            </CardHeader>
            <CardContent className="p-5">
              {loading ? (
                  ...
                  {viewMode !== "voice" && (
                    <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border">
                      <div className="flex items-center gap-3">
                        ...
                        <div><p className="font-bold text-sm text-foreground">Kinesiology Clients</p><p className="text-[10px] font-medium text-muted-foreground">{totals.kineCount} clients · {totals.kineSessions} sessions</p></div>
                      </div>
                      <p className="text-xs font-bold text-muted-foreground">{filteredKine.filter(c => c.lastSession && isAfter(parseISO(c.lastSession), subMonths(now, 1))).length} active (30d)</p>
                    </div>
                  )}
                  {viewMode !== "fnh" && (
                    <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-rose-50 text-rose-600 flex items-center justify-center"><Mail size={14} /></div>
                        <div><p className="font-bold text-sm text-foreground">Voice Students</p><p className="text-[10px] font-medium text-muted-foreground">{totals.voiceCount} students · {totals.voiceLessons} lessons</p></div>
                      </div>
                      <p className="text-xs font-bold text-muted-foreground">{filteredVoice.filter(s => s.lastLesson && isAfter(parseISO(s.lastLesson), subMonths(now, 1))).length} active (30d)</p>
                    </div>
                  )}
                  <div className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center"><Users size={14} /></div>
                      <div><p className="font-bold text-sm text-foreground">Needs Follow-Up</p><p className="text-[10px] font-medium text-muted-foreground">No contact in 60+ days</p></div>
                    </div>
          ...
          <Card className="border-none shadow-lg rounded-[2rem] bg-card overflow-hidden">
            <CardHeader className="p-6 pb-3 border-b border-border bg-muted/30">
              <CardTitle className="text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground flex items-center gap-2">
                <Activity size={14} className="text-emerald-500" /> Client Health — At Risk
              </CardTitle>
            </CardHeader>
            <CardContent className="p-5">
              {loading ? (
                ...
                <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
                  {[...filteredKine.filter(c => c.lastSession && !isAfter(parseISO(c.lastSession), subMonths(now, 2))).map(c => ({
                    ...
                    <div key={`${item.type}-${item.name}-${i}`} className="flex items-center justify-between p-3 bg-muted/30 rounded-xl border border-border">
                      <div className="flex items-center gap-3 min-w-0 flex-1">
                        <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0", item.type === "kine" ? "bg-indigo-50 text-indigo-600" : "bg-rose-50 text-rose-600")}>
                          <User size={14} />
                        </div>
                        <div className="min-w-0">
                      ...
                      <span className={cn("text-[10px] font-black shrink-0 ml-3", item.days > 90 ? "text-rose-500" : item.days > 60 ? "text-amber-500" : "text-muted-foreground")}>
                        {item.days}d
                      </span>
                    </div>
                  ))}
                  {filteredKine.filter(c => c.lastSession && !isAfter(parseISO(c.lastSession), subMonths(now, 2))).length === 0 &&
                   filteredVoice.filter(s => s.lastLesson && !isAfter(parseISO(s.lastLesson), subMonths(now, 2))).length === 0 && (
                    <p className="text-center py-6 text-sm font-bold text-muted-foreground">All clients seen within 60 days. Great work!</p>
                  )}
                </div>
              )}
            </CardContent>
