...
const BusinessOverviewPage = () => {
 ...
 <Card className="border-none shadow-sm rounded-xl bg-card overflow-hidden hover:shadow-sm transition-all group">
 ...
 {loading ? <Skeleton className="h-10 w-28 rounded-xl" /> : <p className="text-3xl font-semibold text-foreground tracking-tight">{value}</p>}
 {sub && <p className="text-xs font-medium text-muted-foreground flex items-center gap-1">{sub}</p>}
 {trend && (
 <p className={cn("text-[10px] font-semibold uppercase tracking-wider flex items-center gap-1", trend.up ? "text-chart-emerald" : "text-destructive")}>
 {trend.up ? <ArrowUpRight size={12} /> : <ArrowDownRight size={12} />} {trend.label}
 </p>
 )}
 </div>
 <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform", color)}>{!loading && Icon}</div>
 </div>
 </CardContent>
 </Card>
 );
 ...
 <Button onClick={togglePrivacy} variant="outline" size="sm" className="rounded-xl h-11 px-4 border-border font-medium text-[10px] uppercase tracking-wider">
 {isPrivate ? <EyeOff size={16} className="mr-2 text-destructive" /> : <Eye size={16} className="mr-2" />}
 ...
 <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
 <MetricCard label="Total Revenue" value={fmt(v.total)} sub={v.sub} icon={<DollarSign size={22} />} color="bg-chart-primary/10 text-chart-primary " trend={{ up: true, label: `${viewMode === 'fnh' ? totals.kineCount : viewMode === 'voice' ? totals.voiceCount : totals.kineCount + totals.voiceCount} paying client${viewMode === 'combined' ? 's' : ''}` }} />
 <MetricCard label="This Week" value={fmt(v.thisWk)} sub={v.thisWkSub} icon={<CalendarDays size={22} />} color="bg-chart-emerald/10 text-chart-emerald " />
 <MetricCard label="This Month" value={fmt(v.thisMo)} sub={v.thisMoSub} icon={<TrendingUp size={22} />} color="bg-muted text-muted-foreground " trend={{ up: monthly.length >= 2 && monthly[monthly.length - 1].kine + monthly[monthly.length - 1].voice >= monthly[monthly.length - 2].kine + monthly[monthly.length - 2].voice, label: monthly.length >= 2 ? `vs ${monthly[monthly.length - 2].month}` : "" }} />
 <MetricCard label="Run Rate" value={fmt(Math.round(v.runRate)) + "/mo"} sub={v.runRateSub} icon={<PiggyBank size={22} />} color="bg-chart-primary/10 text-chart-primary " />
 </div>
 )}

 <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
 <Card className={cn("border-none shadow-sm rounded-xl bg-card overflow-hidden", showPie ? "lg:col-span-3" : "lg:col-span-5")}>
 ...
 <TrendingUp size={16} className="text-primary" /> Weekly Revenue
 </CardTitle>
 </CardHeader>
 <CardContent className="p-8">
 {loading ? <Skeleton className="h-72 rounded-xl" /> : chartData.length > 0 ? (
 ...
 ) : <div className="h-72 flex items-center justify-center text-sm font-medium text-muted-foreground">No revenue data yet.</div>}
 </CardContent>
 </Card>

 {showPie && (
 <Card className="lg:col-span-2 border-none shadow-sm rounded-xl bg-card overflow-hidden">
 <CardHeader className="p-8 pb-4 border-b border-border bg-muted/30">
 <CardTitle className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-3">
 <Wallet size={16} className="text-primary" /> Revenue Split
 </CardTitle>
 </CardHeader>
 ...
 <span className="text-[10px] font-medium text-muted-foreground">{d.name} ({Math.round(d.value / (totals.kine + totals.voice) * 100)}%)</span>
 </div>
 ))}
 </div>
 </div>
 ) : <div className="h-72 flex items-center text-sm font-medium text-muted-foreground">No revenue data.</div>}
 </CardContent>
 </Card>
 )}
 </div>
 ...
 <CardHeader className="p-8 pb-4 border-b border-border bg-muted/30">
 <CardTitle className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-3">
 <Users size={16} className="text-primary" /> Top Spenders — Combined
 </CardTitle>
 </CardHeader>
 ...
 <span className="text-[10px] font-semibold text-muted-foreground w-6 inline-block">{isSameRank ? "—" : i + 1}</span>
 </td>
 <td className="py-4 px-4">
 <p className="font-medium text-sm text-foreground">{isPrivate ? "••••••" : item.name}</p>
 {item.email && <p className="text-[10px] text-muted-foreground font-medium">{isPrivate ? "••••@••••" : item.email}</p>}
 </td>
 <td className="py-4 px-4 text-right">
 <span className="font-semibold text-base">{fmt(Math.round(item.totalSpent))}</span>
 </td>
 <td className="py-4 px-4 text-right">
 <span className={cn("text-[10px] font-semibold uppercase tracking-wider px-3 py-1 rounded-full", item.source === "both" ? "bg-chart-primary/10 text-chart-primary" : item.source === "kine" ? "bg-chart-primary/10 text-chart-primary" : "bg-chart-destructive/10 text-chart-destructive")}>
 ...
 <div className="text-center py-16 text-sm font-medium text-muted-foreground">No revenue data yet.</div>
 )}
 </CardContent>
 </Card>
 )}
 ...
 <CardHeader className="p-8 pb-4 border-b border-border bg-muted/30">
 <CardTitle className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-3">
 <Target size={16} className="text-primary" /> Kinesiology — Active Clients
 </CardTitle>
 </CardHeader>
 <CardContent className="p-6">
 {loading ? (
 ...
 {kineClients.length === 0 && <p className="text-center py-8 text-sm font-medium text-muted-foreground">No paid kinesiology sessions yet.</p>}
 </div>
 )}
 </CardContent>
 </Card>
 ...
 <CardHeader className="p-8 pb-4 border-b border-border bg-muted/30">
 <CardTitle className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground flex items-center gap-3">
 <Target size={16} className="text-destructive" /> Voice Studio — Students
 </CardTitle>
 </CardHeader>
 <CardContent className="p-6">
 {loading ? (
 ...
 <div key={s.email || i} className="flex items-center justify-between p-4 bg-muted/30 rounded-xl border border-border hover:bg-card hover:shadow-md transition-all group">
 <div className="min-w-0 flex-1">
 <p className="font-medium text-sm text-foreground truncate">{isPrivate ? "••••••" : s.name}</p>
 <p className="text-[10px] font-medium text-muted-foreground">{s.lessonCount} lessons · {isPrivate ? "••••@••••" : (s.email || "no email")}</p>
 </div>
 <p className="font-semibold text-base text-foreground shrink-0 ml-4 group-hover:text-chart-destructive transition-colors">{fmt(Math.round(s.totalSpent))}</p>
 </div>
 ))}
 {voiceStudents.length === 0 && <p className="text-center py-8 text-sm font-medium text-muted-foreground">No paid voice lessons yet.</p>}
 </div>
 )}
 </CardContent>
 </Card>
