        ...
        <PageHeader 
          ...
          actions={
            <div className="flex items-center gap-3">
              ...
              {activeTab !== "hub" && (
                <Button variant="outline" onClick={() => handleTabChange("hub")} className="rounded-xl border-border bg-card hover:bg-muted font-bold text-xs uppercase tracking-widest h-12 px-6">
        ...
        {activeTab === "hub" ? (
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            <div className="overflow-x-auto pb-4 no-scrollbar">
              <TabsList className={cn("flex w-max h-auto p-1.5 rounded-2xl gap-1", activeCategory?.bgColor || "bg-muted")}>
                {activeCategory?.items.map((item) => (
                  <TabsTrigger key={item.id} value={item.id} className="rounded-xl py-2.5 px-6 data-[state=active]:bg-card data-[state=active]:text-indigo-600 dark:data-[state=active]:text-indigo-400 data-[state=active]:shadow-sm font-bold text-xs uppercase tracking-widest">
