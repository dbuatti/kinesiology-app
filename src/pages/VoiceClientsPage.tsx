    ...
    <AppLayout variant="workspace">
      <div className="flex flex-col gap-8 pb-32">
            ...
            <div className="flex gap-2">
              <Dialog open={onboardOpen} onOpenChange={setOnboardOpen}>
                <Button
                  ...
                  className="bg-rose-500 hover:bg-rose-600 shadow-xl shadow-rose-200 dark:shadow-rose-900/20 rounded-2xl h-12 px-8 font-black text-xs uppercase tracking-widest"
                >
                  <Plus size={20} className="mr-2" /> Add Student
                </Button>
                <DialogContent className="sm:max-w-[500px] rounded-[2rem] p-0 overflow-hidden bg-white dark:bg-gray-950">
                  <div className="p-8">
                    <DialogHeader className="mb-6">
                      <DialogTitle className="text-2xl font-black">New Student</DialogTitle>
                    </DialogHeader>
                    <VoiceOnboardingForm
                      onSuccess={() => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div className="bg-card rounded-2xl border border-border p-5 flex flex-col gap-1">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Active & Upcoming</span>
            <span className="text-3xl font-black text-emerald-600">{groups.active.length}</span>
          </div>
          <div className="bg-card rounded-2xl border border-border p-5 flex flex-col gap-1">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">1\u20133 Months</span>
            <span className="text-3xl font-black text-amber-600">{groups.oneToThree.length}</span>
          </div>
          <div className="bg-card rounded-2xl border border-border p-5 flex flex-col gap-1">
            <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">3+ Months</span>
            <span className="text-3xl font-black text-rose-600">{groups.threePlus.length}</span>
          </div>
        </div>
        ...
        {bucketConfig.map((cfg) => {
          ...
          return (
            <div key={cfg.key} className={cn("rounded-2xl border overflow-hidden transition-all", cfg.bg, cfg.border)}>
              <div
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="font-black text-sm text-foreground">{cfg.label}</span>
                    <Badge variant="secondary" className="text-[10px] font-black px-2 py-0 rounded-full bg-card border-border">
                      {cfg.count}
                    </Badge>
                  </div>
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            selected
                              ? "border-rose-300 dark:border-rose-700 shadow-sm shadow-rose-200/50"
                        ...
                        >
                          ...
                          >
                            {selected ? <CheckCheck size={20} className="text-rose-500" /> : <Square size={20} />}
                          </button>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              ...
                              {student.hasUpcoming && (
                                <Badge className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border-none shrink-0">
                                  Booked
                                </Badge>
                              )}
                              {student.tags.length > 0 && student.tags.slice(0, 2).map((tag) => (
                                <Badge key={tag} className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-950/30 text-rose-600 dark:text-rose-400 border-none shrink-0">
                                  {tag}
                                </Badge>
                              ))}
                            </div>
                          ...
                          <div className="hidden sm:flex items-center gap-3 text-xs font-medium text-muted-foreground shrink-0">
                            <span className="text-right">
                              <div className="font-bold text-foreground">{formatRelativeDate(student.lastSeenDate)}</div>
                              <div className="text-[10px]">Last seen</div>
                            </span>
                            <span className="text-right">
                              <div className="font-bold text-foreground">{formatRelativeDate(student.lastCommunication ? new Date(student.lastCommunication) : null)}</div>
                              <div className="text-[10px]">Contacted</div>
                            </span>
                            {student.lessonCount > 0 && (
                              <span className="text-right">
                                <div className="font-bold text-foreground">{student.lessonCount}</div>
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <button
                              ...
                              className="p-2 rounded-lg text-muted-foreground hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-all"
                              title="Book a lesson"
                            >
                              <Calendar size={15} />
                            </button>
      ...
      <Dialog open={!!deleteConfirmId} onOpenChange={(o) => !o && setDeleteConfirmId(null)}>
        <DialogContent className="sm:max-w-[400px] rounded-[2rem] bg-white dark:bg-gray-950">
          ...
          <DialogFooter className="flex gap-2 sm:gap-0">
            <Button variant="outline" onClick={() => setDeleteConfirmId(null)} className="rounded-xl font-bold text-xs">
              Cancel
            </Button>
            <Button
              onClick={() => {
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-50 p-4 bg-gradient-to-t from-background via-background to-transparent pointer-events-none">
          <div className="max-w-5xl mx-auto bg-card border border-border rounded-2xl shadow-2xl p-4 flex items-center justify-between gap-4 pointer-events-auto">
            <span className="text-sm font-bold">
              {selectedIds.length} student{selectedIds.length !== 1 ? "s" : ""} selected
            </span>
            <div className="flex items-center gap-2">
              <Button variant="outline" size="sm" className="h-10 rounded-xl border-border font-bold text-xs gap-2" onClick={() => setSelectedIds([])}>
                Clear
              </Button>
              <Button variant="outline" size="sm" className="h-10 rounded-xl border-border font-bold text-xs gap-2" onClick={handleBulkEmail}>
