...
const VoiceOnboardingPage = () => {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 to-white p-4">
        <div className="max-w-md w-full text-center space-y-6 bg-white rounded-[2.5rem] shadow-2xl p-12 border border-rose-100">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 to-white p-4">
      <div className="max-w-lg w-full bg-white rounded-[2.5rem] shadow-2xl border border-rose-100 overflow-hidden">
        ...
        <div className="p-10">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h1 className="text-2xl font-black text-slate-900">Voice Studio</h1>
              <p className="text-sm text-slate-500 font-medium">Complete your student profile</p>
            </div>
          </div>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-700">Email</Label>
              <div className="h-12 rounded-2xl bg-rose-50 border border-rose-200 flex items-center px-4 text-sm font-bold text-slate-700">
                {decodedEmail}
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-700">
                Mobile Number <span className="text-slate-400 font-normal normal-case tracking-normal">(optional)</span>
              </Label>
              <Input
                value={mobile}
            ...
            <div className="space-y-2">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-700">
                Goals <span className="text-slate-400 font-normal normal-case tracking-normal">(what do you want to work on?)</span>
              </Label>
              <Textarea
                value={goals}
                onChange={(e) => setGoals(e.target.value)}
                placeholder="e.g. Improve breath support, expand vocal range, prepare for a performance..."
                className="min-h-[100px] rounded-2xl border-slate-200 text-sm font-medium resize-none"
              />
            </div>

            <div className="space-y-2">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-700">Experience Level</Label>
              <select
                value={experienceLevel}
                onChange={(e) => setExperienceLevel(e.target.value)}
                className="flex h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-sm font-medium"
              >
            ...
            <div className="space-y-2">
              <Label className="text-xs font-black uppercase tracking-widest text-slate-700">
                Additional Notes <span className="text-slate-400 font-normal normal-case tracking-normal">(optional)</span>
              </Label>
              <Textarea
                value={additionalNotes}
                onChange={(e) => setAdditionalNotes(e.target.value)}
                placeholder="Any medical considerations, preferences, or questions..."
                className="min-h-[80px] rounded-2xl border-slate-200 text-sm font-medium resize-none"
              />
            </div>
