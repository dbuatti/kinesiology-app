...
serve(async (req) => {
  ...
  try {
    ...
    // Same fallback pattern as send-manual-onboarding/voice-send-onboarding —
    // the portal link below needs the real domain, not a bare relative path
    // (a client can't click "/portal/login" in an email; it has no host).
    // A voice-focused conversation gets the friendlier studio.* domain
    // (VOICE_SITE_URL) instead of the kinesiology one, so a piano/voice
    // student's emailed link doesn't say "kinesiology" anywhere — real
    // feedback: "piano voice students will get confused with the
    // kinesiology part of the domain". Only resolvable here for a FOCUSED
    // conversation (voice_student_email known for the whole thread); a
    // general (unfocused) conversation drafting to a voice student found
    // via a tool call mid-turn still falls back to the kinesiology domain —
    // a real but rarer case than the focused one this fixes.
    const SITE_URL = (voice_student_email ? Deno.env.get("VOICE_SITE_URL") : null)
