...
serve(async (req) => {
  console.log("--- [v24] DOUBLE DB SYNC (APPTS + PLANNER) START ---");
  ...
  try {
    ...
    // 4. SUPABASE RECORD (Linking to the Planner Page ID)
    ...
    await supabase.from('appointments').insert({
      ...
      notion_page_id: plannerData.id // Stores the finance page ID
    });
