// @ts-nocheck
// Emails a client a cancellation note (editable template) from the timetable.
...
async function sendGmail(accessToken: string, from: string, to: string, subject: string, html: string) {
  ...
  const raw = [
    `From: ${from}`, `To: ${to}`, "MIME-Version: 1.0",
...
serve(async (req) => {
  ...
  try {
    const { to, message, startISO } = await req.json();
    ...
    if (startISO) {
      ...
      if (!isNaN(d.getTime())) {
        const fd = d.toLocaleDateString("en-AU", { weekday: "long", day: "numeric", month: "long", timeZone: "Australia/Melbourne" });
        const ft = d.toLocaleTimeString("en-AU", { hour: "numeric", minute: "2-digit", hour12: true, timeZone: "Australia/Melbourne" });
        dateLine = `<div style="background-color:#FFF1F2;border-radius:16px;padding:16px 20px;margin:20px 0;border:1px solid #FECDD3;color:#9F1239;font-size:14px;"><s>${fd} · ${ft}</s> — cancelled</div>`;
      }
    }

    const bodyHtml = esc(message).split(/\n+/).map((p) => `<p style="margin:0 0 14px;">${p}</p>`).join("");

    const subject = "About your session — a change of plan";
    const html = `
      <!DOCTYPE html><html>
