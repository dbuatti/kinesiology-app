CREATE TABLE IF NOT EXISTS public.limiting_belief_sessions (
  ...
  dissolve_log JSONB DEFAULT '[]'::jsonb,
...
DROP POLICY IF EXISTS "Users can only see their own sessions" ON public.limiting_belief_sessions;
DROP POLICY IF EXISTS "Users can only insert their own sessions" ON public.limiting_belief_sessions;
DROP POLICY IF EXISTS "Users can only update their own sessions" ON public.limiting_belief_sessions;
DROP POLICY IF EXISTS "Users can only delete their own sessions" ON public.limiting_belief_sessions;
